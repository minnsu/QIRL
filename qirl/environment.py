...
class AdvancedEnvironment:
    ...
    def __init__(self, chart_data, price_idx, init_cash=10000000, min_rate=0.05, max_rate=0.2, hold_interval=0.1, reward_scaler=100):
        ...
        self.record = [[0, 0., 0.], [0, 0., 0.], [0, 0., 0.]] # [n_buys, buy_profit, buy_loss], [n_sells, sell_profit, sell_loss], [n_holds, hold_profit, hold_loss]
    ...
    def step(self, action): # (action, confidence)
        ...
        if reward > 0:
            self.record[action][1] += float(reward)
        else:
            self.record[action][2] += float(reward)
        ...
        if self.idx >= len(self.chart_data):
            ...
            info['[buy, profit, loss]'] = [self.record[0][0], self.record[0][1], self.record[0][2]]
            info['[sell, profit, loss]'] = [self.record[1][0], self.record[1][1], self.record[1][2]]
            info['[hold, profit, loss]'] = [self.record[2][0], self.record[2][1], self.record[2][2]]
            self.reset()
        else:
            self.state = self.chart_data[self.idx]
            self.train_state = self.train_data[self.idx]
